...
###############################################################################
        ...
        res_message = TextSendMessage(text='💁💁客服在線時間: \n周一~周六 10:00 A.M~17:00 P.m \n若沒有及時回復代表真人客服忙線中，煩請耐心等待回復~ 您也可以透過以下聯絡方式聯絡本店 \n電話：123-456-789 \nEmail：hoppyshop@gmali.com \n本店地址：台北市00路123號 \n如有任何問題，請隨時與我們聯繫。')
        line_bot_api.reply_message(event.reply_token, res_message)
        return 0
###############################################################################
        # user_message='文字訊息'
...
###############################################################################
        ...
        res_message = TextSendMessage(
            text='⚠️未滿500不出貨呦⚠️ \nHOBBY模型店之模型清單連結: \nhttps:// hobbyShop.com.tw/modletooldm.rtg2fdgdg6e3asg \n歡迎貴客選購，本店歡迎使用線上購物~')
        line_bot_api.reply_message(event.reply_token, res_message)
        return 0
###############################################################################
        # user_message='文字訊息'
